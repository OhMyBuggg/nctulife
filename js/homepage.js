...
*/
    ...
    function change_point(){
        if( n==1 ){
            ...
            $(".hp-nav li").css("background-color","white")//除了被點擊到的游標，其他都恢復成原來的顏色
            $(".hp-nav li:eq(4)").css("background-color","#646464")
        }
    }
    //根據捲軸的位置改變右方導覽列游標的顏色
    $(window).scroll(function(){
         if(moving==0 && $(window).scrollTop()>=$(".hp-p01").offset().top && $(window).scrollTop()<$(".hp-p02").offset().top){
            ...
            $("#hp-field-font3").addClass("hp-field-link-after2").removeClass("hp-field-link-before")
            *///n=3
        }else if(moving==0 && $(window).scrollTop()>=$(".hp-p04").offset().top && $(window).scrollTop()<$(".hp-p05").offset().top){
            //$(".hp-nav li").css("background-color","white")//除了被點擊到的游標，其他都恢復成原來的顏色
            //$(".hp-nav li:eq(3)").css("background-color","#646464")
            n=4;
            change_point();
            //n=4
        }else if(moving==0 && $(window).scrollTop()>=$(".hp-p05").offset().top){
            //$(".hp-nav li").css("background-color","white")//除了被點擊到的游標，其他都恢復成原來的顏色
            //$(".hp-nav li:eq(4)").css("background-color","#646464")
            n=5;
            change_point();
    ...
    for(i=0;i<=num_li;i++){
        $(".hp-nav li:eq("+i+")").click({id:i},function(e){
            ...
            $(".hp-nav li:eq("+e.data.id+")").css("background-color","#646464")//被點擊到的游標變色，前面的selector用this也可以
